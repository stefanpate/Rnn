...
class rnn:
	def __init__(self, n_units, n_inputs, n_outputs, p_con, dale, p_inh, g, taus, activation_fcn, seed=None, restored_vars=None):
		...
		self.trainable_vars = [self.bias, self.w, self.w_out]
	...
	def init_vars(self):
		...
		if self.restored_vars != None:
			bias_init, w_init, w_out_init = self.restored_vars # Restore in alphabetical order. Imposed in helper fcn "load_trainable"
			bias_init, w_init, w_out_init = bias_init.reshape(self.n_outputs, 1), w_init.reshape(self.n_units, self.n_units), w_out_init.reshape(self.n_outputs, self.n_units)
		else:
