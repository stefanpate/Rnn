...
class rnn:
	def __init__(self, n_units, n_inputs, n_outputs, p_con, dale, p_inh, g, taus, activation_fcn, seed=None, restored_vars=None):
		...
		self.restored_vars = restored_vars # bias, taus_gaus, w, w_out

		w_init, w_out_init, taus_gaus_init, bias_init = self.init_vars()
		...
		# Trainable
		self.bias = tf.Variable(bias_init, name="bias", shape=(self.n_outputs, 1))
		...
		self.trainable_vars = [self.bias, self.taus_gaus, self.w, self.w_out]
	...
	def simulate(self, u, trial_len, batch_size=1):
		...
		x = tf.random.normal(shape=(self.n_units, batch_size)) # Initial state of syncur
		...
		for t in range(trial_len):
			...
			next_x = tf.multiply((1 - self.delta_t / taus_sig), x)\
				 ...
				 + tf.random.normal(shape=(self.n_units, batch_size)) / 10
			next_o = tf.matmul(self.w_out, self.activation_fcn_tf(next_x)) + self.bias
			x = next_x
		...
		if batch_size == 1: # Make 2D array for test mode (batch_size=1)
			output = tf.reshape(output, shape=(self.n_outputs, trial_len))
			syncur = tf.reshape(syncur, shape=(self.n_units, trial_len))
	...
	def init_vars(self):
		...
		if self.restored_vars != None:
			...
			bias_init, taus_gaus_init, w_init, w_out_init = bias_init.reshape(self.n_outputs, 1), taus_gaus_init.reshape(self.n_units, 1),\
															w_init.reshape(self.n_units, self.n_units), w_out_init.reshape(self.n_outputs, self.n_units)
		else:
	...
	def get_output(self, x):
		...
		o = np.matmul(self.w_out.numpy(), self.activation_fcn_np(x)) + self.bias.numpy()
		return o