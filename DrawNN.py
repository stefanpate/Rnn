...
12. Filename with target drawing (fn)
Example command:
python DrawNN.py 800 0.25 0.2 1 1 True True False 22 0 0.9 j3_norm_concat
'''
# Model settings
...
fn = str(sys.argv[12])
file_path = f"/home/spate/PaintApp/{fn}.csv" # Path to drawing
file = np.loadtxt(file_path, delimiter=',')
trial_len = file.shape[0]
...
threshold = 0.05 # Mean squared error
...
hyperparams = {"fn":fn, "n_units":n_units, "n_inputs":n_inputs, "n_outputs":n_outputs, "p_con":p_con,\
...
if dotrain:
	...
	while (ep < max_ep) & (not met_threshold): # Go till performance threshold met
		u, z = draw(file_path=file_path, n_outputs=n_outputs) # Get stim, target
		# u, z = sine(trial_len)
...
if dotest:
	...
	print(f"Completed {test_eps} testing episodes.")
